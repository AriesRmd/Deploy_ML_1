...
def predict_model_comparation():
    class_list = {'Cercospora': 0, 'Common Rust': 1, 'Northern Leaf Blight': 2, 'Healthy':3}
    file = request.files["file"]
    file.save(os.path.join('static', 'temp.jpg'))
    img = cv2.cvtColor(np.array(Image.open(file)), cv2.COLOR_BGR2RGB)

    model = load_model('static/MLModule/model_1.h5')
    ...
    modelh = load_model('static/MLModule/model_3.h5')
...
def predict_select_image():
    ...
    model_dict = {
        'model_1'         :   'static/MLModule/model_1.h5',
        'model_2'        :   'static/MLModule/model_3.h5'
        }
